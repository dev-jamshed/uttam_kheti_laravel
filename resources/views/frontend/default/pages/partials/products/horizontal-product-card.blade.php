...
@endif
    ...
    <div class="thumbnail  rounded-2">
        ...
        <div
            class="product-overlay position-absolute  d-flex align-items-center justify-content-center gap-1">
    ...
    <div class="card-content mt-4 mt-sm-0 w-100">
        ...
        @if (count(generateVariationOptions($product->without_variation_combinations)) > 0)
            @foreach (generateVariationOptions($product->without_variation_combinations) as $variation)
                ...
                <ul
                    class="product-radio-btn mt-1 mb-3 d-flex align-items-center justify-content-center gap-2 @if ($loop->last) mb-4 @endif">
                    @if ($variation['id'] != 2)
                        @foreach ($variation['values'] as $value)
                            <li>
                             
                                <label for="val-just_view{{ $value['id'] }}">{{ $value['name'] }}</label>
                            </li>
                        @endforeach
                    @else
                        <!-- color -> id=2 -->
